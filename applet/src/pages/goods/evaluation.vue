    ...
    <view class="he-page-content">
        <view class="he-top">
            ...
            <view class="flex flex-wrap" :data-theme="theme">
                <view class="he-tab__item" @click="setTab(item.key)" v-for="(item, index) in tabs"
                      :class="tabKey === item.key ? 'he-tab__item-1' : 'he-tab__item-0'" :key="index">
            ...
            <view class="he-item" v-for="item in list" :key="item.id">
                <view class="he-item__header flex align-center justify-between">
                    ...
                    <view class="he-item__stars flex">
                        <text v-for="num in 5" :key="num" class="iconfont iconevaluate_star"
                              :class="item.star >= num ? 'he-star__1' : 'he-star__0'"></text>
                    </view>
                </view>
                <view class="he-item__body">
                    ...
                    <view class="he-item__pictures flex flex-wrap" v-if="!$h.test.isEmpty(item.images)">
                        <he-image :height="210" :width="210" mode="aspectFill" class="he-item__img" :image-style="{
                            borderRadius: '8rpx'}" @click="doPreviewImage(img, item.images)"
                                  v-for="(img, imgIndex) in item.images" :key="imgIndex" :src="img"></he-image>
                    </view>
        ...
        <he-no-content-yet v-if="isNothing" text="暂无评价"
                           :image="ipAddress + '/evaluation-background-empty.png'"></he-no-content-yet>
    </view>
</template>
...
export default {
    ...
    data() {
        return {
            tabs: [{key: 'all', name: '全部'}, {key: 'image', name: '有图'}, {key: 'good', name: '好评'}, {
                key: 'general',
                name: '中评'
            }, {key: 'bad', name: '差评'}],
            list: [],
