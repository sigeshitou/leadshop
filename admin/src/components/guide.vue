<template>
    <div class="guide" v-if="!is_close">
        <div v-if="step===index" v-for="item,index in masks" :key="index">
            <div class="guide-mask" :style="{top:position.top,left:position.left,width:item.width,height:item.height}"></div>
            <div class="guide-animation" :style="{top:position.top,left:position.left}">
                <img :src="item.src">
            </div>
        </div>
        <div class="guide-step">
            <img :src="guideNext" @click="toStep">
            <p @click="toStop">跳过引导</p>
        </div>
    </div>
</template>
